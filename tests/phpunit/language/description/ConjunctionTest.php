...
class ConjunctionTest extends DescriptionTest {
	...
	public function isSingletonProvider() {
		...
		$argLists[] = array(
			new Conjunction( array( new \Ask\Language\Description\ValueDescription( new \DataValues\StringValue( 'ohi' ) ) ) ),
			true
		);
		...
		$argLists[] = array(
			new Conjunction( array( new Conjunction( array() ) ) ),
			false
		);
	 ...
	 * @since 0.1
	 */
	public function testIsSingleton( Conjunction $description, $isSingleton ) {
